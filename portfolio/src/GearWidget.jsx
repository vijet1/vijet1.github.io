...
export default function GearWidget(){
  ...
  const [renderForce, sendRenderForce] = useState(0);
  const designSize = 400;

  useEffect(() => {
    const sketch = (p) => {
      const gears = [[], [], []];
      const designSize = p.random(200,700);
      p.setup = () => {
        const { offsetWidth, offsetHeight } = containerRef.current;
        p.createCanvas(offsetWidth, offsetHeight);
        p.colorMode(p.HSB, 360, 100, 100, 100);

        const scale = Math.min(p.width, p.height) / designSize;
      ...
      p.draw = () => {
        ...
        p.scale(Math.min(p.width, p.height) / designSize);
  ...
  function handlePress(){
    sendRenderForce(renderForce + 1);
  }

  return (
  <div className="gears" role="button" tabIndex={0} onClick={handlePress} onKeyDown={handlePress}>
    <div className='TIME'>
      <Clock />
    </div>
    <div ref={containerRef} className="gearsClip" >
    </div>
  </div>
  );
}